  ...
  <div class="w-full mx-auto items-center flex justify-between md:flex-nowrap flex-wrap md:px-16 px-4">
    <div>
      ...
      <p class="mt-2 md:mt-3 text-sm md:text-base leading-relaxed text-white opacity-80">
        Silakan isi data pengeluaran barang dari inventaris.
      </p>
    </div>
  </div>
</div>
...
<div class="px-6 -mt-32">
  ...
  <div class="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded">
    <div class="rounded-t px-6 py-3 border-0 bg-blueGray-50">
      <h3 class="font-semibold text-base text-blueGray-700">Form Tambah Pengeluaran</h3>
    </div>

    <div class="px-6 py-6">
      <form method="POST" action="{{ route('staff.pengeluaran.store') }}">
        @csrf
        ...
        <div class="mb-4">
          ...
          <select id="barangSelect" name="barang_id" required
            ...
            @foreach ($barangs as $barang)
              <option value="{{ $barang->id }}">{{ $barang->nama_barang }} (Stok: {{ $barang->ketersediaan }})</option>
            @endforeach
          </select>
        </div>
        ...
        {{-- Jumlah --}}
        <div class="mb-4">
          <label class="block text-sm font-medium text-blueGray-600 mb-1">Jumlah Keluar</label>
        ...
        <div class="mb-4">
          ...
          <input type="text" name="dipakai_oleh" required value="{{ old('dipakai_oleh') }}"
            class="block w-full text-sm border rounded px-4 py-2">
        </div>
        ...
        <div class="mb-4">
          ...
          <input type="date" name="tanggal_pengeluaran" required value="{{ old('tanggal_pengeluaran') }}"
            class="block w-full text-sm border rounded px-4 py-2">
        </div>
