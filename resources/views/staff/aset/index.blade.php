  ...
  <div class="w-full mx-auto items-center flex justify-between md:flex-nowrap flex-wrap md:px-16 px-4">
    <div>
      ...
      <p class="mt-2 md:mt-3 text-sm md:text-base leading-relaxed text-white opacity-80">
        Berikut adalah daftar seluruh aset tetap inventaris.
      </p>
    </div>
  </div>
</div>
...
<div class="px-6 -mt-32">
  ...
  {{-- Card Table --}}
  <div x-data="liveAset()" x-init="fetchTable()" class="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded">
    ...
    <div class="rounded-t px-4 py-3 border-b border-blueGray-100 bg-blueGray-50 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <h3 class="font-semibold text-base text-blueGray-700">Tabel Aset Tetap</h3>
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div class="relative">
          ...
          <input type="text" x-model="search" placeholder="Cari aset..."
  ...
  function liveAset() {
    return {
      ...
      fetchTable() {
        ...
        fetch(`{{ route('staff.aset.index') }}?${params.toString()}`, {
          headers: {
...
{{-- SweetAlert untuk sukses & konfirmasi hapus (boleh di-reuse persis) --}}
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    deleteButtons.forEach(button => {
      button.addEventListener('click', function (e) {
        ...
        const nama = this.getAttribute('data-nama') || 'aset ini';
