  ...
  <div class="w-full mx-auto items-center flex justify-between md:flex-nowrap flex-wrap md:px-16 px-4">
    <div>
      ...
      <p class="mt-2 md:mt-3 text-sm md:text-base leading-relaxed text-white opacity-80">
        Silakan isi data barang yang ingin ditambahkan ke dalam sistem inventaris.
      </p>
    </div>
  </div>
</div>
...
<div class="px-6 -mt-32">
  ...
  <div class="relative flex flex-col min-w-0 break-words bg-white w-full mb-6 shadow-lg rounded">
    <div class="rounded-t px-6 py-3 border-0 bg-blueGray-50">
      <h3 class="font-semibold text-base text-blueGray-700">Form Tambah Barang</h3>
    </div>
    ...
    <div class="px-6 py-6">
      <form method="POST" action="{{ route('staff.barang.store') }}">
        ...
        <div class="mb-4">
          ...
          <input type="text" name="subkategori" value="{{ old('subkategori') }}"
            class="w-full border px-4 py-2 rounded text-sm focus:outline-none focus:ring-2 focus:ring-lightBlue-500" required>
        </div>
        ...
        <div class="mb-6">
          ...
          <input type="number" name="tahun_masuk" value="{{ old('tahun_masuk') }}"
            class="w-full border px-4 py-2 rounded text-sm focus:outline-none focus:ring-2 focus:ring-lightBlue-500" min="1900" max="{{ date('Y') }}">
        </div>
