  ...
  <div class="w-full mx-auto flex flex-col md:flex-row justify-between items-start md:items-center md:px-16 px-4 gap-6">
    ...
    <form method="GET" class="flex flex-wrap gap-2 items-end">
      <div>
        ...
        <select name="bulan" class="appearance-none px-4 py-2 pr-10 border text-sm rounded focus:ring-2 focus:ring-lightBlue-500">
          @php
            $bulanList = [
      ...
      <div>
        ...
        <select name="tahun" class="appearance-none px-4 py-2 pr-10 border text-sm rounded focus:ring-2 focus:ring-lightBlue-500">
      ...
      <div class="flex gap-2 pt-[22px]">
        <button type="submit" class="bg-white text-lightBlue-500 font-bold text-xs px-4 py-2 rounded shadow hover:bg-lightBlue-100">
          Tampilkan
        </button>
        <a href="{{ route('staff.report.export', ['bulan' => $bulan, 'tahun' => $tahun]) }}"
...
<div class="px-6 relative z-0 transform translate-y-[3.2rem] pb-28">
  <div class="grid md:grid-cols-3 gap-6 mb-6">
    <div class="bg-white p-4 rounded shadow">
      ...
      <p class="text-lg font-bold text-emerald-600 mt-2">Rp {{ number_format($totalPengadaan, 0, ',', '.') }}</p>
    </div>
    <div class="bg-white p-4 rounded shadow">
  ...
  <div class="mb-6">
    ...
    <div class="bg-white rounded shadow overflow-x-auto max-h-[500px] overflow-y-auto">
      <table class="w-full table-auto border-collapse">
        ...
        <tbody>
          @forelse ($pembelian as $pengadaan)
            <tr class="hover:bg-blueGray-50">
              <td class="px-6 py-4 text-sm">{{ \Carbon\Carbon::parse($pengadaan->tanggal_pengadaan)->format('d-m-Y') }}</td>
              <td class="px-6 py-4 text-sm">{{ $pengadaan->barang->nama_barang }}</td>
              <td class="px-6 py-4 text-sm">{{ $pengadaan->jumlah }}</td>
              <td class="px-6 py-4 text-sm">Rp {{ number_format($pengadaan->harga_satuan, 0, ',', '.') }}</td>
              <td class="px-6 py-4 text-sm font-semibold">Rp {{ number_format($pengadaan->jumlah * $pengadaan->harga_satuan, 0, ',', '.') }}</td>
            </tr>
          @empty
            <tr><td colspan="6" class="text-center text-blueGray-400 text-sm py-4">Tidak ada data pembelian.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
        ...
        <tbody>
          @forelse ($perbaikan as $rusak)
            <tr class="hover:bg-blueGray-50">
              ...
              <td class="px-6 py-4 text-sm text-yellow-600 font-semibold">Rp {{ number_format($rusak->biaya_perbaikan, 0, ',', '.') }}</td>
              <td class="px-6 py-4 text-sm">{{ $rusak->catatan_perbaikan ?? '-' }}</td>
            </tr>
          @empty
            <tr><td colspan="4" class="text-center text-blueGray-400 text-sm py-4">Tidak ada data perbaikan.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
