<table class="items-center w-full bg-transparent border-collapse">
  <thead class="sticky top-0 bg-blueGray-100 z-0">
    <tr>
      ...
      <th class="px-6 py-3 text-xs font-semibold text-blueGray-500 uppercase border-b text-left">Harga Satuan</th>
      <th class="px-6 py-3 text-xs font-semibold text-blueGray-500 uppercase border-b text-left">Total Harga</th>
      <th class="px-6 py-3 text-xs font-semibold text-blueGray-500 uppercase border-b text-left">Supplier</th>
  ...
  <tbody>
    @forelse ($pengadaans as $item)
      <tr class="hover:bg-blueGray-50">
        ...
        <td class="px-6 py-4 text-sm border-t">{{ $item->supplier ?? '-' }}</td>
        <td class="px-6 py-4 text-sm border-t">{{ \Carbon\Carbon::parse($item->tanggal_pengadaan)->format('d M Y') }}</td>
        <td class="px-6 py-4 text-sm border-t">{{ $item->keterangan ?? '-' }}</td>
        <td class="px-6 py-4 text-sm border-t">
          <div class="flex items-center space-x-2">
            <a href="{{ route('staff.pengadaan.edit', $item->id) }}" class="text-blue-500 hover:text-blue-700 text-sm" title="Edit">
              <i class="fas fa-edit"></i>
            </a>
            <form action="{{ route('staff.pengadaan.destroy', $item->id) }}" method="POST" class="inline">
              @csrf @method('DELETE')
    ...
    @empty
      <tr>
        <td colspan="8" class="px-6 py-4 text-center text-blueGray-400 text-sm">Belum ada data pengadaan.</td>
      </tr>
    @endforelse
  </tbody>
</table>
